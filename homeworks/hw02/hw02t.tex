...
\intermediatesubproblem{$R^2$ is commonly interpreted as \qu{proportion of the variance explained} and proportions are constrained to the interval $\zeroonecl$. While it is true that $R^2 \leq 1$, it is not true that $R^2 \geq 0$. Construct an explicit example $\mathbb{D}$ and create a linear model $g(x) = w_0 + w_1 x$ whose $R^2 < 0$. Hint: do not use the OLS line. Hint: draw a picture!}\spc{10}
