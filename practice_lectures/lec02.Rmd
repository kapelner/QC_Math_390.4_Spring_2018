...
We can "make up" a dataset using the sampling we just learned. We will also introduce the "matrix" data type below:
...
We can also name rows and columns
...
Let's pretend salary is normally distributed with mean \$50,000 and standard error \$20,000. Let's 


Let's pretend only 5% of people don't pay back their loans. 

Also, 50\% of people have no crime, 40\% have an infraction, 8\% a misdimeanor and 2\% a felony.





It would be nice to keep factor or character information in a matrix... but...


So the key data type is the "dataframe". It is an upgraded matrix.


It worth knowing a couple more data structures in R. These are not "data [science] types", these are "[computer science] data types". 

The first is arrays which are multidimensional vectors
...
The second are "lists" which are hashmaps.
