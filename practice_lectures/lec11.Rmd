...
What did we learn here? This model really shouldn't be used outside of the range of $\mathbb{D}$.

Is the linear model of median home value using rooms *actually* linear? Not a chance! You cannot have rooms be negative or more than e.g. 50. And you can't have median values that are negative.

Linear models are usually only linear in a sensical range of the predictors.
...
And are the residuals summed / averages to be zero?

```{r}
sum(e)  #numerically 0
mean(e) #numerically 0
```
...
Q[, 1] %*% Q[, 2]^2 #orthogonal? Numerical error piles up...
Q[, 2] %*% Q[, 3]^2 #orthogonal? Numerical error piles up...

yhat_via_Q = Q %*% t(Q) %*% y
sum(abs(yhat - yhat_via_Q)) #they are numerically the same
```
