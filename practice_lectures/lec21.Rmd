...
Let's return to the linear logistic model with all features. Let's calculate the overall oos reward for each possible $p_{th}$:

```{r}
performance_metrics_oos = data.frame(performance_metrics_oos)
performance_metrics_oos$tot_reward = 
...
ggplot(performance_metrics_oos) +
  geom_line(aes(x = p_th, y = tot_reward)) + 
  ylim(0, max(performance_metrics_oos$tot_reward)) #don't show models that lose money at all
```
...
```{r}
i_star = which.max(performance_metrics_oos$tot_reward)
