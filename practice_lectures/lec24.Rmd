...
Let's see a classification example, the adult data. the linear model comparison will be logistic regression with the default hard classification threshold of $p_{th} = 0.5$.
...
test_indices = sample(setdiff(1 : nrow(adult), training_indices), 3000)
adult_test = adult[test_indices, ]
y_test = adult_test$income
X_test = adult_test
X_test$income = NULL

mod_lin = glm(y_train ~ ., X_train, family = "binomial")
y_hat_test_lin = predict(mod_lin, X_test, type = "class")
s_e_lin = sd(y_test - y_hat_test_lin)
s_e_lin
...
```{r}
bagged_tree_mod = YARFBAG(data.frame(x = x_train), y_train, num_trees = num_trees, calculate_oob_error = TRUE) #TRUE is the default! Why? It's something you want to know!
bagged_tree_mod
```
